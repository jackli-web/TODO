...
<script>
import Headzj from './components/headzj.vue';

import shopzj from "./components/shopzj.vue";
export default {
  components: { shopzj, Headzj },
  name: 'App',
  methods:{
    add(index){
      console.log( this.info[index].count);
      this.info[index].count++;
    },
    del(index){
      if (this.info[index].count==0) return;
      this.info[index].count--;
    }
  },
  data() {
